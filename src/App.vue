...
<template>
  <div class="mainContent">
    ...
    <div class="container-handler">
      ...
      <button class="button-85" @click="resetHandler">Reset game</button>
  </div>
</template>
...
// Cắt tỉa alpha-beta
    MiniMax(s, d, mp) {
      // giá trị của mỗi node là 0 1 -1 nên apha = -2 và beta = 2
      return this.AlphaBeta(s, d, -2, 2, mp);
    },

    AlphaBeta(s, d, a, b, mp) {
      console.log("vao s", s);
      // neu node là nút lá hoặc độ sâu d = 0 thì trả về giá trí của node đó là thắng thua hoặc hoà
      if (this.isEndNode(s) || d == 0) {
        return this.Value(s);
      }
      let sz = s.size;

// nếu đang ở trạng thái max thì duyệt qua tất cả các đỉnh con
      if (mp) {
        for (let i = 0; i < sz; i++) {
          for (let j = 0; j < sz; j++) {
            ...
            // nếu node = true thì đang ở max, nếu đanng ở maximum thì phải truyền vào false(các node của nó đang ở trạng thái min)
            // gọi a là maximum của tất cả các đỉnh con của node hiện tại
            a = Math.max(a, tmp);
            // nếu a > = b thì xoá luôn nút đó
            if (a >= b) {
              break;
            }
          }
    ...
    checkShow(index) {
      ...
      if (this.textShow != "" && this.textShow != "Match Drawn") {
